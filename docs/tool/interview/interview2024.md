...
rootFontSize = screenWidth / baseValue；
...
1. 在js的`分析`阶段，会进行`上下文初始化：创建作用域链、创建变量对象、创建this指向`；在`创建对象`的时候会首先进行`函数声明`，并将函数名指向函数在内存中的地址；之后通过`var`创建的变量会进行`变量声明`，赋值为`undefined`；`通过let和const创建的变量`不会进行变量提升，只有在声明语句执行后才会被赋值并访问。
2. 等到上下文进行`执行`阶段再对已经声明的变量进行赋值，这就是js中出现变量提升和函数提升原因。


- **暂时性死区（TDZ）**: 使用 let 和 const 声明的变量在声明之前无法被访问的区域。这个区域**从代码块开始到变量声明语句之间**，被称为"暂时性死区"。


**作用域：**

1. let、const可以形成块级作用域，var不会形成块级作用域: {} （es6新增）; `函数`全局、函数、块级作用域都会形成
...
- 标记阶段（Mark Phase）：垃圾回收器从 根对象 开始遍历，标记所有能够访问到的对象，未被标记的对象就是"垃圾"；
- 清除阶段（Sweep Phase）：遍历堆内存中的所有对象，清除所有未被标记的对象，释放这些对象占用的内存空间
...
1. 迭代对象的方式:
- `for...in`: `用于遍历对象的可枚举属性`，包括对象自身的属性以及继承的属性。在迭代对象时，for...in 循环会遍历对象的键（即`属性名`）。
- `for...of`: `用于遍历可迭代对象`（比如数组、字符串、Set、Map 等），遍历对象的可迭代的属性值。在迭代对象时，for...of 循环会遍历`对象的值`。
...
> 当我们中止请求后，网络请求就会变成`canceled`状态；后端可能照常把计算/写库/调用下游做完 ，只是最后回包时发现客户端已断开，结果发不出去或被丢弃。
...
`Web Worker`
- 作用 ：把计算放到后台线程跑，避免阻塞主线程（UI 卡顿、输入延迟、掉帧）。
- 典型场景 ：大计算（加密/压缩/解析大文件/图像处理/复杂排序）、长任务拆分、把 CPU 密集逻辑从主线程移走。
- 特点/限制 ：
    - 不能直接操作 DOM、 window 、多数同步 API
    - 与主线程通过 postMessage / message 通信
    - 生命周期一般跟页面相关：页面关了通常就没了（SharedWorker 例外）


`Service Worker`
- 作用 ：一个“运行在浏览器与网络之间的代理脚本”，核心能力是 拦截网络请求 （fetch 事件）并做`缓存、离线、预加载、请求改写`等，是 PWA 的基础。
- 典型场景 ：
    - `离线可用（Cache API）`
    - 性能优化（资源缓存、stale-while-revalidate）
    - 统一网络层策略（鉴权刷新、降级、A/B、日志上报策略等）
    - 推送通知、后台同步（Push/Background Sync，视浏览器支持）
- 特点/限制 ：
    - 不能操作 DOM
    - 必须 HTTPS （localhost 例外）
    - 有 作用域 scope （通常控制某个路径下的页面）
    - 生命周期是“事件驱动 + 可被浏览器随时终止再唤醒”，不是常驻进程；适合短任务，不适合长时间占用 CPU
    - 常用存储：`Cache Storage、IndexedDB`
...
3. diff算法提升: vue3对于静态的标签和属性会作`静态标记`，在 diff 过程中会直接跳过静态节点，从而提升 diff 效率。
4. 写法改变：
...
> chunk 是`依赖图里的模块按 entry、import()、splitChunks、runtime 策略分出来的可加载单元`，所以它和入口、动态导入边界、公共依赖抽取策略、runtime 配置、模块依赖关系强相关。
...
> 某个 chunk 最终包含的模块代码 + webpack 运行时代码/模块 ID 等生成结果 变化了，它对应的 chunkhash 就会变。
...
- Loader是webpack用来处理各种类型文件的`转换器`，主要负责将非JavaScript文件转换为webpack能够处理的模块，在`模块加载时`对文件内容进行预处理；可以链式调用，从右到左、从下到上执行

- Plugin是webpack的`扩展机制`，用于执行更广泛的任务，可以在`webpack构建过程的各个阶段`执行自定义逻辑，主要用于优化、资源管理、环境变量注入等
...
1. 采集阶段：数据的采集, 开发者可以通过 window.performance 属性获取。
    - 页面性能情况：
        ...
        - FMP: 首次有效绘制；页面渲染过程中元素增量最大的点，因为元素增量最大的时候，页面主要内容也就一般都渲染完成了；
...
- 传统CSR（客户端渲染）的渲染流程：`浏览器请求 → 下载HTML（空壳） → 下载JS → 执行JS → 渲染页面`；首屏渲染时间 = `网络请求时间 + JS下载时间 + JS执行时间`

- SSR的渲染流程：`浏览器请求 → 服务器渲染完整HTML → 返回带内容的HTML → 立即显示`；

1. 减少关键渲染路径
- CSR ：HTML → CSS → JS → 渲染
- SSR ：HTML（已包含内容）→ 渲染

2. 并行化资源加载：SSR返回的HTML已包含内容，JavaScript可以在后台异步加载，不阻塞页面渲染
3. 优化网络利用率：服务器端渲染时可以并行获取数据，一次请求返回完整页面，减少往返次数，避免客户端的"瀑布式"请求
...
### 为什么 Node.js 在很多高并发 I/O 场景里表现好

- `事件循环 + 非阻塞 I/O `：Node 的核心是单线程事件循环（Event Loop）。当请求大部分时间在“等 I/O”（等数据库、等下游 HTTP、等 Redis），线程不会被阻塞住，而是继续处理其他连接上的事件回调/Promise，从而用较少线程支撑大量并发连接。
- `更低的线程/上下文切换开销` ：传统“一个连接/请求对应一个线程”的模型在高并发时会产生大量线程、栈内存占用和上下文切换开销。Node 依赖少量线程（主线程 + libuv 线程池处理部分阻塞任务）通常更省。
- `生态对异步 I/O 友好` ：HTTP/WS、反向代理、BFF、网关层、实时推送等场景，Node 的编程模型和库非常成熟，容易把吞吐做上去。


### 什么时候 Node.js 并不“更佳”

- `CPU 密集型` （加密/压缩/图像处理/复杂计算/大 JSON 序列化等）：Node 主线程被计算占住，事件循环就会卡顿，表现为延迟飙升、吞吐下降。此时 Go/Java/Rust 等更容易利用多核并行，或需要把计算下沉到 worker_threads/子进程/独立服务。
- `需要强并行的多核利用` ：Node 单进程默认难以吃满多核，通常要用 cluster /多进程/容器横向扩展；而一些语言运行时天然多线程并行更直接。
- `性能瓶颈常在外部依赖` ：高并发系统更多瓶颈在数据库连接池、锁、缓存命中率、下游限流、网络、序列化协议等；换成 Node 不会自动变快。

在 `I/O 密集、异步友好的高并发场景`，Node.js 往往能以更低资源成本取得很高吞吐和连接承载能力 ；但`在 CPU 密集或需要强多核并行 的场景`，它不占优势，需要配合多进程/worker 或改用更适合的技术栈。
...
while (true) {
    ...
    if (done) break; // 结束
    const chunk = decoder.decode(value, { stream: true }); // 获取当前块的文本内容
    text += chunk; // 前端进行文本拼接
}
```
...
Nginx 和 Node.js 在很多方面是类似的，例如都是 HTTP 服务器、事件驱动、异步非阻塞等，且 Nginx 拥有的功能，也可以使用 Node.js 去实现，但它们的使用场景是不同的，`Nginx 擅长于底层服务器端资源的处理（静态资源处理转发、反向代理，负载均衡等），Node.js 更擅长上层具体业务逻辑的处理.`
...
2. 访问com`顶级域名服务器`，它再告诉你`baidu.com`域名服务器的地址；
3. 最后访问apple.com`权威域名服务器`，就得到了`www.baidu.com`的地址。


浏览器输入一个域名，完整的解析流程：

`浏览器缓存 -> 操作系统缓存 -> hosts文件 -> 非权威域名服务器（公司自己的 DNS 服务器） -> 根域名服务器 -> 顶级域名服务器 -> 权威域名服务器。`
...
3. TCP三次握手建立连接：
    ...
    - 服务端接收到请求后向客户端发送一个 `SYN ACK报文段`，确认连接请求，并且也向客户端发送一个`随机序号`。
...
> 浏览器收到 Set-Cookie 后，会按该 Cookie 的 Domain/Path 等作用域保存；之后只有在请求的域名、路径等满足匹配规则（以及 Secure/SameSite 等条件）时，才会自动在请求头 Cookie 中携带并发送给服务器；不是给所有域名请求都发送。
...
- 小程序的逻辑层和渲染层是分开的，逻辑层运行在 `JSCore` 中，并没有一个完整的浏览器对象，所以缺少相关的 `DOM API 和 BOM API`。微信小程序中虽然不能直接操作DOM，但通过 wx.createSelectorQuery() 等API可以
...
1. `安装速度和性能`：pnpm通常被认为是最快的，因为它采用了`只下载必需的模块`的方法，而不是下载整个依赖树。此外，`pnpm还可以并行下载模块`，从而进一步提高下载速度。yarn也使用`并行安装和缓存机制`来提高安装速度，尤其是当之前已经安装过某个软件包时，yarn会从缓存中获取，而不是重新下载。相比之下，npm按照队列依次安装每个包，安装速度可能较慢。
...
3. `兼容性和可靠性`：npm是Node.js的官方包管理器，因此它具有最好的兼容性。yarn和pnpm都与npm兼容，但在使用旧版本的Node.js时可能会遇到一些问题。`yarn`被认为比npm和pnpm更可靠，因为它使用了`多线程下载和安装`，减少了下载和安装失败的风险。此外，yarn还使用锁定文件（如yarn.lock）来确保安装的模块与项目的依赖项相匹配，这也有助于提高可靠性。

4. `功能和扩展性`：`yarn`提供了一些额外的功能，如`缓存、自动解析和自动重试`等，这些功能可以提高开发效率。npm和pnpm也有一些额外的功能，但它们的功能不如yarn丰富。此外，yarn和pnpm都支持插件系统，可以通过安装插件来扩展其功能。
...
``` js
// 重新赋值location：
window.location.href = window.location.href + '?t=' + Date.now(); // 添加时间戳强制刷新
...
默认拿不到。两者跨站（不同站点/域名）。在 iframe 场景里，B 的 zhihu.com Cookie 对顶层站点 baidu.com 来说通常属于“第三方上下文”，现代浏览器会强限制第三方 Cookie。

Cookie 按域名隔离：请求 zhihu.com 时浏览器只会带 zhihu.com （及其允许的 Domain/Path）下的 Cookie，不会把 baidu.com 的 Cookie 带给 zhihu.com 。同源策略也阻止 B 通过 JS 直接读取 A 的 document.cookie 。
