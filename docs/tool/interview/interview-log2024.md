...
``` json
// 如何保证编译的测试环境跟线上环境是一致的？
1. 容器化部署: Docker统一环境
2. 环境变量管理：统一配置文件，.env文件管理
3. 依赖版本锁定：package-lock.json/yarn.lock，Node.js版本管理
4. CI/CD流水线，GitHub Actions配置
5. 构建配置统一：Webpack配置
```
...
``` json
### 技术选型
- 框架选择 ：React/Vue/Angular，考虑团队技术栈和项目复杂度
- 构建工具 ：Webpack/Vite/Rollup，权衡构建速度和功能需求
- 状态管理 ：Redux/Zustand/Pinia，根据数据流复杂度选择
### 架构设计
- 分层架构 ：UI层、业务逻辑层、数据访问层清晰分离
- 模块化 ：按功能或业务域划分模块，降低耦合度
- 组件设计 ：原子化设计，建立设计系统和组件库
### 工程化考虑
- 代码规范 ：ESLint、Prettier、Git Hooks确保代码质量
- CI/CD ：自动化测试、构建、部署流程
- 监控体系 ：错误监控、性能监控、用户行为分析
### 性能优化
- 首屏加载 ：代码分割、懒加载、预加载策略
- 缓存策略 ：浏览器缓存、CDN、Service Worker
- 包体积 ：Tree Shaking、按需加载、资源压缩
```
3. 对了很多b端的项目，从架构的角度考虑，有哪些思考点？微前端实现子项目之间通信隔离的原理是什么？WebView嵌入怎么做性能优化？
``` json
// 对了很多b端的项目，从架构的角度考虑，有哪些思考点？
### 权限管理
- RBAC模型 ：角色-权限-资源三层权限控制
- 路由守卫 ：页面级和组件级权限控制
- 按钮级权限 ：细粒度的操作权限管理
### 数据管理
- 表格组件 ：支持排序、筛选、分页的通用表格
- 表单验证 ：统一的表单验证规则和错误处理
- 数据缓存 ：合理的数据缓存策略，减少重复请求
### 用户体验
- 加载状态 ：骨架屏、Loading动画提升感知性能
- 错误处理 ：友好的错误提示和降级方案
- 操作反馈 ：及时的成功/失败反馈
### 可维护性
- 配置化 ：页面配置化，减少重复开发
...
- 资源压缩 ：Gzip、Brotli压缩
### 渲染优化
- 首屏优化 ：SSR、骨架屏、关键CSS内联
- 懒加载 ：图片、组件按需加载
- 虚拟滚动 ：长列表性能优化
### Native交互优化
- JSBridge优化 ：减少Native调用频次
- 数据预取 ：Native层预先获取数据
- 离线缓存 ：关键页面离线可用
### 内存管理
- 页面回收 ：及时释放不用的页面
...
// 路由匹配时才加载对应的组件
function RouteComponent({ path, component: Component }) {
...
- 核心功能模块设计：权限管理系统、数据管理层
- 工程化配置：构建优化、代码质量保障
- 性能优化策略：渲染优化、缓存策略
- 安全性设计：前端安全措施，XSS, CSRF,敏感数据,权限控制
- 监控与运维：错误监控、性能监控、用户行为分析
- 部署与发布：CI/CD流程、容器化部署、负载均衡
```
2. ssr能够缩短首屏渲染时间的底层原理是什么？
``` json
1. 预渲染机制 ：将组件渲染工作从客户端转移到服务端，减少客户端等待时间
2. 数据预取 ：在服务端完成数据获取，避免客户端的额外网络请求
3. 关键渲染路径优化 ：减少阻塞渲染的资源加载，实现内容的即时可见
4. 并行处理 ：HTML 内容渲染和 JS 加载并行进行，而非串行等待
5. 客户端水合（Hydration）机制: 获取服务端传递的初始状态,创建与服务端相同的 store, 水合现有的 DOM 结构
  - 水合过程底层原理：遍历现有的 DOM 节点，与 Virtual DOM 进行对比，绑定事件监听器， 建立 React 内部的引用关系， 如果不匹配，替换节点
```
3. 封装一个虚拟滚动列表组件需要做哪些事情？哪些场景需要用防抖节流，哪些场景不需要使用？
``` json
- 基础架构设计: 数据源、项目高度、容器高度、渲染函数、预渲染数量、滚动回调...
- 核心计算逻辑：计算可视区域范围、计算总高度、计算偏移量、 滚动处理（防抖优化）
```
4. 手写题：js浅比较的代码实现
...
``` js
基于 CI/CD 的自动部署方案, ci.yml、deploy.sh、Docker 容器化部署
守护进程：守护进程是在后台运行的进程，不依赖于终端会话，即使用户退出登录也能继续运行。pm2、forever
> PM2集群模式 + 系统级服务 + 自动重启机制

- 零停机部署策略：滚动更新（pm2 restart）
- 负载均衡和扩容：Nginx 负载均衡配置，自动扩容脚本：
const os = require('os');
...
setInterval(async () => {
  ...
  if (loadPercentage > 80) {
    // 负载过高，增加实例
    await pm2.scale('my-app', '+1');
    console.log('负载过高，增加一个实例');
  } else if (loadPercentage < 30) {
    // 负载较低，减少实例
    await pm2.scale('my-app', '-1');
    console.log('负载较低，减少一个实例');
  }
}, 60000); // 每分钟检查一次
- 流量控制和限流
- 监控和告警：健康检查端点、性能监控
...
``` json
- 文本提取自动化 ：从代码中自动识别和提取需要翻译的文本
- 翻译流程自动化 ：集成翻译服务，减少人工翻译工作量
- 部署更新自动化 ：翻译更新后自动同步到各个环境
1. 自动化提取 ：AST 解析 + Babel 插件，自动识别需翻译文本
2. 智能翻译 ：集成多个翻译服务，AI 辅助翻译，保证质量
3. 流程自动化 ：CI/CD 集成，自动提取、翻译、验证、部署
4. 质量保证 ：术语一致性、格式检查、人工审核流程
5. 性能优化 ：按需加载、缓存策略、分块打包
6. 管理工具 ：可视化管理后台，支持批量操作和协作
```
3. 代码题：
``` js
// 1. 执行顺序：
...
``` json
- 网络层面 ：资源加载时间、请求数量、传输大小
- 渲染层面 ：首屏渲染时间、关键渲染路径
- 代码层面 ：包体积、执行效率、加载策略
- 缓存层面 ：浏览器缓存、CDN缓存、应用缓存
```
...
``` json
- 解析阶段：vue-loader解析.vue文件
- 模板编译：模板编译成渲染函数
- 脚本处理： script块直接提取，添加模板渲染函数
- 样式处理：postcss处理css，添加浏览器前缀
- 输出：生成的js文件
```
2. 手写题：
``` js
...
注水是指在客户端接收到服务端渲染的HTML后，React重新"激活"这些静态HTML，使其具备交互能力的过程。
```
