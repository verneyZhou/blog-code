...
1. 保证 state 和 props 的一致性；props 必然异步, 因为只有当父组件重渲染了我们才知道 props
...
### 为什么React v18之前setState在合成事件中是异步的，在原生事件中是同步的？v18又做了什么改变？
...
`ReactDOM.render`：这是React中最常用的渲染方法。当你想要显式渲染一个React组件到DOM中时，你会使用这个方法。它会直接渲染给定的React元素到指定的容器DOM元素中，并替换容器内的所有内容。这种方式是`同步`的，意味着它会阻塞其他代码的执行直到渲染完成。
...
1. 数组的元素是按次序排列的, `解构赋值的灵活性 数组解构允许我们自由命名变量`，
...
useState 返回数组相比于对象会更灵活、解构起来也会更简洁、方便，避免了对象属性名的限制和冲突。如果返回对象，多个 useState 就会有命名冲突的问题
...
- `组件层面优化`: 
  - React.memo ：防止函数组件不必要的重新渲染
  - useMemo ：缓存计算结果
  - useCallback ：缓存函数引用
- `渲染优化`
  - 避免内联对象和函数 ：内联每次渲染都创建新对象，可提取到外部
  - 合理使用key ：帮助React识别列表项变化
- `状态管理优化`：状态下沉 ：将状态放在最近的公共父组件；状态分割 ：避免单一大状态对象
- `代码分割和懒加载`：React.lazy + Suspense
- `虚拟化长列表`：使用 react-window 或 react-virtualized
- `图片和资源优化`：图片懒加载、代码压缩、CDN加速
- `开发工具辅助`
  - React DevTools Profiler ：分析组件渲染性能
  - Chrome DevTools ：监控内存使用和性能
  - Bundle Analyzer ：分析打包体积
- `服务端渲染` ：使用 Next.js 或自建 SSR 提升首屏性能
