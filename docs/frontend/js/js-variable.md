...
打印顺序为：
``` js
ƒ a(){} 111 // 块级作用域内，函数提升；a和window.a是不同的变量
222 111 // 块级作用域内的a被重新赋值为222，但是window.a还是111
222 222 // 在执行到 function a(){} 时，会做一次“向外层 var 绑定同步”的动作，浏览器会把 此刻块内 a 的值（222） 同步给外层的 a （全局/函数作用域的那个）。
// 浏览器常见的 Annex B 同步时机： 进块先有块内函数绑定；运行到声明语句时才把“块内当前值”同步到外层 。
```
...
这里的 “Annex B” 通常指ECMAScript 规范的附录 B（Additional ECMAScript Features for Web Browsers） ：为兼容历史网页而保留的一些“非理想但现实存在”的语义。

“同步时机”，一般就是在问： `块里的 function f(){} 什么时候会影响到外层（ var 环境/全局对象）的那个 f ？`

常见的 Annex B “同步/生效时机”：
- 创建名字（binding）时机：脚本初始化阶段就可能先把名字挂出来 （类似 var f 的“先声明”），因此外层可以看到一个名为 f 的绑定，但值一开始通常是 undefined 。
- 把函数值“同步”到外层绑定的时机：只有当运行时真的进入并执行到这个块时才发生 （也就是分支被走到、块被执行时），这时才会把外层的 f 赋值为该函数对象。
``` js
console.log(typeof f); // 常见结果：'undefined'
if (true) {
  function f() {}
}
console.log(typeof f); // 常见结果：'function'
```
