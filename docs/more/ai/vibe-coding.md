...
Harness Engineering 的本质就是`建设一个工程系统，让 Agent 稳定地读、做、验证、迭代，并在长期运行中保持可控`。

Harness层包含什么？至少四个核心子系统：
- `约束文档`（告诉模型"你是谁、你能做什么、你不能做什么"）
- `记忆系统`（跨会话保留关键信息）
- `上下文管理`（在有限的context window里决定保留什么、丢弃什么）
- `反馈循环`（当Agent犯错时如何检测并纠正）
...
Rules 是什么？Rules 是写给 AI 看的「持久约束」，每次生成代码时都会自动生效。相当于把团队的`编码规范、技术约定、禁止事项`提前告诉 AI。

Skills 是什么？Skills 管的是「教 AI 做某件具体的事」。Skills 是`可复用的任务模板，把一类重复性的生成任务封装起来，下次直接调用`。
...
Spec Coding、 Rules、Skill上面讲的这些，其实本质都是为了解决“`上下文`”问题
...
::: tip 标准开发流程总结
1. 接到需求
2. 整理上下文：技术栈、数据结构、约束、边界条件
3. 让 AI 出技术方案（不写代码）
4. 确认方案 + 让 AI 提问补充信息
5. 分模块逐步生成代码
6. 每个模块 Review 通过后再继续下一个
7. 集成联调
8. 提测
:::
...
前端开发 SOP 最好只管 5 件事：
1. 输入：开始前需要哪些信息。
2. 边界：哪些文件、行为、依赖不能随便改。
3. 步骤：先读什么，再改什么，最后查什么。
4. 验收：功能、样式、状态、响应式、命令怎么检查。
5. 回复：最后必须把改动、验证和风险说清楚。
...
[我把前端开发 SOP 写进 Skill，Codex 终于不乱发挥了](https://mp.weixin.qq.com/s/1ogkc_tYA1PzS7uVXuwxtQ)
...
- [前端工程师 20 个 Skill，帮你把 AI 接手陌生项目时的跑偏率降下来](https://mp.weixin.qq.com/s/2Wx65adkj28ota6d063oww)
